        ...
        if staff_menu == "Manage Appointments":
            ...
            if not df.empty:
                ...
                for i, row in df.iterrows():
                    with st.expander(f"📌 {row['patient_name']} | {row['department']} | {row['date']} | Status: {row['status']} | Stage: {row['stage']} | Ref: {row['booking_ref']}"):
                        st.write(f"📱 Phone: {row['phone']} | Doctor: {row['doctor']} | Notes: {row['notes'] if row['notes'] else 'None'}")
                        ...
                        with cols[2]:
                            ...
                            if st.button("🔄 Update Stage", key=f"updatestage{row['id']}"):
                        ...
                        with cols[3]:
                            if st.button("✏️ Edit", key=f"edit{row['id']}"):
                                with st.form(f"editform{row['id']}"):
                                    new_name = st.text_input("Name", value=row['patient_name'])
                                    new_phone = st.text_input("Phone", value=row['phone'])
                                    new_dept = st.text_input("Department", value=row['department'])
                                    new_doc = st.text_input("Doctor", value=row['doctor'])
                                    ...
                                    if save_edit:
                                        c.execute("""UPDATE appointments SET patient_name=?, phone=?, department=?, doctor=?, date=?, updated_at=? WHERE id=?""",
                                                  (new_name, new_phone, new_dept, new_doc, str(new_date), datetime.now().strftime('%Y-%m-%d %H:%M:%S'), row['id']))
                                        conn.commit()
                                        rerun_flag = True
                        # Reminder
                        with cols[4]:
                            if st.button("📩 Reminder", key=f"remind{row['id']}"):
                                send_notification(row['phone'], f"Reminder: Your appointment ({row['booking_ref']}) is scheduled for {row['date']}")
                        ...
                        # Notes
                        new_note = st.text_area("Add Notes", value=row['notes'] if row['notes'] else "", key=f"note{row['id']}")
                        if st.button("Save Notes", key=f"savenote{row['id']}"):
                            c.execute("UPDATE appointments SET notes=?, updated_at=? WHERE id=?", (new_note, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), row['id']))
                            conn.commit()
            ...
            st.download_button("Download CSV", df.to_csv(index=False), "appointments.csv")
            st.download_button("Download Excel", df.to_excel("appointments.xlsx", index=False), "appointments.xlsx")
